...
<main class="max-w-screen-xl mx-auto px-6 py-8 space-y-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-5 space-y-6">
            ...
            {{-- SECTION 4: INLINE DYNAMIC AI CHATBOT SYSTEM --}}
            <section>
                <p class="text-xs font-medium text-crimson-600 uppercase tracking-widest mb-3">
                    {{ $chatbot->label ?? 'SFAC Campus AI Assistant' }}
                </p>

                <div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">

                    {{-- Chat header --}}
                    <div class="bg-crimson-600 text-white px-4 py-3 flex items-center gap-2">
                        <i class="fa-solid fa-robot text-sm"></i>
                        <span class="text-sm font-medium">{{ $chatbot->name ?? 'FrancisAI' }}</span>
                        <span class="ml-auto w-2 h-2 rounded-full bg-sfacgold-400"></span>
                    </div>

                    {{-- Message area --}}
                    <div id="chatMessages" class="h-60 overflow-y-auto px-4 py-4 space-y-3 bg-gray-50/50">
                        {{-- AI welcome message seeded from DB --}}
                        <div class="flex gap-2 items-start">
                            <div class="w-6 h-6 rounded-full bg-crimson-50 flex-shrink-0 flex items-center justify-center mt-0.5">
                                <i class="fa-solid fa-robot text-crimson-600 text-[10px]"></i>
                            </div>
                            <div class="bg-white border border-gray-200 rounded-t-xl rounded-br-xl px-3 py-2 max-w-sm">
                                <p class="text-xs text-gray-700 leading-relaxed">
                                    {{ $chatbot->welcome_message ?? 'Hello! I\'m FrancisAI, your SFAC campus assistant. I can help with enrollment, academic schedules, campus policies, and more.' }}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- Input bar --}}
                    <div class="border-t border-gray-100 flex items-center gap-0">
                        <input
                            id="chatInput"
                            type="text"
                            placeholder="Ask FrancisAI anything about SFAC…"
                            class="flex-1 text-sm px-4 py-3 bg-white focus:outline-none placeholder-gray-400 text-gray-800"
                            onkeydown="if(event.key==='Enter') sendChat()"
                        />
                        <button
                            onclick="sendChat()"
                            class="bg-crimson-600 hover:bg-crimson-700 text-white text-xs font-medium px-5 py-3 transition-colors duration-150"
                        >
                            Send
                        </button>
                    </div>
                ...
                {{-- Hidden config data parameters managed via dashboard backend settings panel --}}
                <div
                    id="chatbotConfig"
                    data-model="{{ $chatbot->model ?? 'gpt-4o' }}"
                    data-system-prompt="{{ $chatbot->system_prompt ?? 'You are FrancisAI, the helpful campus assistant for Saint Francis of Assisi College (SFAC). Always be polite, concise, and helpful.' }}"
                    data-max-tokens="{{ $chatbot->max_tokens ?? 512 }}"
                    class="hidden"
                ></div>
            </section>
        </div>
    ...
    function sendChat() {
        ...
        messages.innerHTML += `
            <div class="flex gap-2 items-start justify-end">
                <div class="bg-crimson-50 border border-crimson-100 rounded-t-xl rounded-bl-xl px-3 py-2 max-w-sm">
        ...
        // Read dynamic config parameters
        const systemPrompt = config.dataset.systemPrompt;
        const model        = config.dataset.model;
        const maxTokens    = parseInt(config.dataset.maxTokens, 10) || 512;

        // Typing indicator layout logic
        ...
        messages.innerHTML += `
            <div id="${typingId}" class="flex gap-2 items-start">
                ...
                <div class="bg-white border border-gray-200 rounded-t-xl rounded-br-xl px-3 py-2">
                    <p class="text-xs text-gray-400 italic">FrancisAI is typing…</p>
                </div>
            </div>`;
        messages.scrollTop = messages.scrollHeight;

        // POST connection straight to backend proxy controller route
        fetch('/api/chatbot/message', {
            ...
            body: JSON.stringify({ message: text, system: systemPrompt, model, max_tokens: maxTokens }),
        })
        .then(r => r.json())
        .then(data => {
            ...
            messages.innerHTML += `
                <div class="flex gap-2 items-start">
                    ...
                    <div class="bg-white border border-gray-200 rounded-t-xl rounded-br-xl px-3 py-2 max-w-sm">
                        <p class="text-xs text-gray-700 leading-relaxed">${escapeHtml(reply)}</p>
                    </div>
                </div>`;
            messages.scrollTop = messages.scrollHeight;
        })
        .catch(() => {
            ...
            messages.innerHTML += `
                <div class="flex gap-2 items-start">
                    ...
                    <div class="bg-white border border-gray-200 rounded-t-xl rounded-br-xl px-3 py-2 max-w-sm">
                        <p class="text-xs text-red-400 leading-relaxed italic">Connection error. Please try again.</p>
                    </div>
                </div>`;
            messages.scrollTop = messages.scrollHeight;
        });
...
<div id="chatbot-popup" class="fixed right-5 bottom-20 z-50 hidden w-80 max-w-full rounded-3xl border border-gray-200 bg-white shadow-2xl">
    ...
    <div id="chatbot-messages" class="h-64 overflow-y-auto px-4 py-4 space-y-3 bg-gray-50"></div>
...
<button onclick="openChatbot()" class="fixed right-5 bottom-5 z-40 rounded-full bg-crimson-600 p-4 text-white shadow-lg hover:bg-crimson-700">
...
<script>
    ...
    function appendMessage(sender, text) {
        ...
        msg.innerHTML = `${sender === 'Bot' ? 'SFACY' : sender}: ${escapeHtml(text)}`;
