...
<head>
    ...
    <title>Edit Announcement — SFAC</title>
...
<main class="max-w-4xl mx-auto px-6 py-10">
    ...
    <form method="POST" action="{{ route('announcements.update', $announcement) }}" class="rounded-[2rem] border border-red-100 bg-white shadow-xl">
        @csrf
        @method('PUT')
        ...
        <div class="grid gap-8 p-8">
            <section class="rounded-3xl border border-red-100 bg-red-50 p-6">
                ...
                <p class="mt-2 text-sm text-red-700/80">Update the announcement title and content.</p>

                <div class="mt-6 space-y-6">
                    <div>
                        <label class="block text-sm font-medium text-red-900">Title</label>
                        <input type="text" name="title" value="{{ old('title', $announcement->title) }}" required class="mt-2 w-full rounded-3xl border border-red-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-red-600 focus:ring-2 focus:ring-red-100" />
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-red-900">Body</label>
                        <textarea name="body" rows="6" required class="mt-2 w-full rounded-3xl border border-red-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-red-600 focus:ring-2 focus:ring-red-100">{{ old('body', $announcement->body) }}</textarea>
                    </div>
                </div>
            </section>
            ...
            <section class="rounded-3xl border border-slate-200 bg-white p-6">
                        ...
                        <div class="mt-3 grid gap-3">
                            ...
                            @foreach ($types as $key => $label)
                                <label class="flex items-center gap-3 rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 transition hover:border-red-300">
                                    <input type="radio" name="type" value="{{ $key }}" {{ old('type', $announcement->type) === $key ? 'checked' : '' }} class="h-4 w-4 text-red-600 focus:ring-red-500" />
                                    <span>{{ $label }}</span>
                                </label>
                            @endforeach
                        </div>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <select name="urgency" required class="mt-2 w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-red-600 focus:ring-2 focus:ring-red-100">
                                <option value="low" {{ old('urgency', $announcement->urgency) == 'low' ? 'selected' : '' }}>Low</option>
                                <option value="medium" {{ old('urgency', $announcement->urgency) == 'medium' ? 'selected' : '' }}>Medium</option>
                                <option value="high" {{ old('urgency', $announcement->urgency) == 'high' ? 'selected' : '' }}>High</option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-slate-900">Published date</label>
                            <input type="datetime-local" name="published_at" value="{{ old('published_at', optional($announcement->published_at)->format('Y-m-d\TH:i')) }}" class="mt-2 w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-red-600 focus:ring-2 focus:ring-red-100" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-slate-900">Publication deadline</label>
                            <input type="datetime-local" name="deadline" value="{{ old('deadline', optional($announcement->deadline)->format('Y-m-d\TH:i')) }}" class="mt-2 w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-red-600 focus:ring-2 focus:ring-red-100" />
                            <p class="mt-2 text-xs text-slate-500">Leave blank to publish immediately, or set a future deadline to defer publication.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6">
                    <label class="flex items-center gap-3 rounded-3xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-900">
                        <input type="checkbox" name="is_published" value="1" {{ old('is_published', $announcement->is_published) ? 'checked' : '' }} class="h-4 w-4 text-red-600 focus:ring-red-500" />
