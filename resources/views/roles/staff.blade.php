...
<head>
    ...
    <title>Staff/Faculty Dashboard - SFAC</title>
...
<header class="bg-crimson-600 text-white">
    <div class="max-w-screen-xl mx-auto px-6 py-3 flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="font-display text-base font-normal leading-tight tracking-wide">SFAC Faculty Dashboard</h1>
                <p class="text-xs text-white/60 leading-none mt-0.5">Faculty & Staff Portal</p>
            </div>
        </div>

        <div class="flex items-center gap-3">
            <span class="text-sm text-white/80"><i class="fa-solid fa-user-tie mr-1"></i> {{ ucfirst(auth()->user()->role) }}</span>
            <div class="relative">
...
<main class="max-w-screen-xl mx-auto px-6 py-8 space-y-8">
    ...
    <section>
        <h2 class="text-4xl font-bold text-gray-800 mb-4">Faculty & Staff Dashboard</h2>
        <p class="text-sm text-gray-600 mb-6">Welcome, {{ auth()->user()->name }} — manage your courses and post announcements.</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <a href="{{ route('announcements.index') }}" class="group bg-white rounded-xl border border-gray-200 p-6 hover:border-crimson-600 hover:shadow-md transition-all duration-200 flex flex-col h-full">
                ...
                <p class="text-xs text-gray-500">Create, edit, and manage announcements for campus</p>
            ...
            <a href="{{ route('achievements.index') }}" class="group bg-white rounded-xl border border-gray-200 p-6 hover:border-crimson-600 hover:shadow-md transition-all duration-200 flex flex-col h-full">
                ...
                <h3 class="text-sm font-semibold text-gray-800 mb-1">Achievements Board</h3>
                <p class="text-xs text-gray-500">Create and view student achievements and awards</p>
                <div class="text-xs text-crimson-600 font-medium mt-3">
                    <i class="fa-solid fa-square-plus mr-1"></i> Manage
                </div>
            </a>
            ...
            <!-- Student Profiles card removed for staff/faculty access; only admins may manage student accounts -->
        </div>
    </section>
        ...
        <div class="space-y-3">
            @forelse ($announcements->take(5) as $ann)
                <div class="text-sm border-l-2 border-crimson-200 pl-3">
                    ...
                    <p class="text-xs text-gray-500">{{ $ann->published_at->format('M d, Y') }} • Posted by {{ $ann->user?->name ?? 'Unknown author' }}</p>
                </div>
            @empty
